...
### 翻訳範囲
- **対象**: 操作UI全般＋使い方/解説モーダルの本文（見出し・本文・箇条書き・注意書き・図キャプション・計算表ヘッダ）。
- **対象（動的図ラベル）**: 厳密ケガキ図でJSが生成するラベル（断面図/底面図のタイトル、面なし・面一・面二・テーパー壁、「3面目以降は省略」注記）は `t()` 化して翻訳。
- **対象外**: 静的SVG（`images/*.svg` および help内のインライン作図 = ケガキの実例/側面のケガキ等）に焼き込まれた文字、寸法の数値、アプリ名「側溝カットガイド」。
